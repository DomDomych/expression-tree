...
void ExpressionTree::setVariable(const std::string &name,
...
int NumberNode::evaluate(const VariableStorage &) const { return value; }
...
int NumberNode::height() const { return 1; }

int VariableNode::height() const { return 1; }

int BinaryOperatorNode::height() const {
  return std::max(left->height(), right->height()) + 1;
}

bool ExpressionTree::build_expression_tree(
  ...
  for (int i = 0; i < tokens.size(); i++) {
    if (isOperator(tokens[i])) {
      ...
      stack.push(std::make_unique<BinaryOperatorNode>(
          tokens[i], std::move(left), std::move(right)));
...
int NumberNode::operatorsCount() const { return 0; }

int VariableNode::operatorsCount() const { return 0; }
...
std::string NumberNode::toPostfix() const { return std::to_string(value); }
std::string NumberNode::toInfix() const { return std::to_string(value); }
std::string NumberNode::toPrefix() const { return std::to_string(value); }

std::string VariableNode::toPostfix() const { return name; }
std::string VariableNode::toPrefix() const { return name; }
std::string VariableNode::toInfix() const { return name; }

std::string BinaryOperatorNode::toPostfix() const {
  std::string result =
      left->toPostfix() + " " + right->toPostfix() + " " + op + " ";
  return result;
}

std::string BinaryOperatorNode::toInfix() const {
  std::string result =
      "( " + left->toInfix() + " " + op + " " + right->toInfix() + " )";
  return result;
}

std::string BinaryOperatorNode::toPrefix() const {
  std::string result =
      op + " " + left->toPrefix() + " " + right->toPrefix() + " ";
...
bool NumberNode::isConstant() const { return true; }

bool VariableNode::isConstant() const { return false; }
...
NumberNode::simplified(const VariableStorage &) const {
  return std::make_unique<NumberNode>(value);
}

std::unique_ptr<ExprNode>
VariableNode::simplified(const VariableStorage &Storage) const {
  return std::make_unique<VariableNode>(name);
}

std::unique_ptr<ExprNode>
BinaryOperatorNode::simplified(const VariableStorage &Storage) const {
  auto newLeft = left->simplified(Storage);
  auto newRight = right->simplified(Storage);

  auto node = std::make_unique<BinaryOperatorNode>(op, std::move(newLeft),
                                                   std::move(newRight));
...
void ExpressionTree::clear() { root = nullptr; }