...
protected:
  ...
  unsigned int height(Node *p) { return p ? p->h : 0; }

  int bfactor(Node *p) {
    return static_cast<int>(height(p->right)) -
           static_cast<int>(height(p->left));
  }
...
class AVLTree<std::string, std::string>
    : public AVLTreeCommon<std::string, std::string> {
...
private:
  ...
  void
  find_by_word(Node *p, const std::string &word,
...
public:
  ...
  find_by_word(const std::string &word) const {
