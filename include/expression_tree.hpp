...
public:
  ...
  virtual std::unique_ptr<ExprNode>
  simplified(const VariableStorage &) const = 0;
};
...
public:
  BinaryOperatorNode(const std::string &, std::unique_ptr<ExprNode> left,
                     std::unique_ptr<ExprNode> right);
...
private:
  ...
  bool infix_to_postfix(const std::vector<std::string> &tokens,
